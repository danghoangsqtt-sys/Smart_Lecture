...
if (classId) {
  ...
  check('teaching preflight inventories only the selected class and subject', readiness.status === 200 && readiness.data?.scope?.classId === classId && readiness.data?.scope?.subjectId === subjectId && readiness.data?.curriculum?.linkedLectureCount >= 1 && readiness.data?.materials?.pptxCount >= 1 && readiness.data?.powerPointConversion?.required === true && [true, false, null].includes(readiness.data?.powerPointConversion?.available));
