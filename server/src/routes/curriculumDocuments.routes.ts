...
router.post('/classes/:classId/curriculum-documents', requireRole('teacher','admin'), upload.single('file'), h(async (req,res) => { const cls=access(String(req.params.classId),(req as AuthedRequest).user!); const file=req.file; const subjectId=String(req.body.subjectId||''); const ext=file ? path.extname(file.originalname).toLowerCase() : ''; if (!file || !['.pdf','.docx'].includes(ext) || !db.prepare('SELECT 1 FROM subjects WHERE id=? AND class_id=?').get(subjectId,cls.id)) throw new HttpError(400,'BAD_INPUT','Chọn môn học và tệp PDF/DOCX'); const stored=`${randomUUID()}${ext}`; renameSync(file.path,path.join(MEDIA_DIR,stored)); const id=randomUUID(); db.prepare('INSERT INTO curriculum_documents (id,class_id,subject_id,title,file_path,mime_type,size_bytes) VALUES (?,?,?,?,?,?,?)').run(id,cls.id,subjectId,String(req.body.title||path.basename(file.originalname,ext)),stored,file.mimetype,file.size); res.status(201).json({id}); }));
