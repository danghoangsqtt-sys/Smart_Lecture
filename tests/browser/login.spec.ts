...
test('default circuit room restores host and learners without duplicate grading', async ({ page, request, browser }) => {
  ...
  await page.getByRole('button', { name: 'Chấm ngay và chuyển bài' }).click();
  await expect(page.getByRole('heading', { name: 'Tổng kết học tập mạch' })).toBeVisible();
  const learningDebrief = page.getByLabel('Tổng kết học tập mạch');
  await expect(learningDebrief).toContainText('17%');
  await expect(learningDebrief).toContainText('3/18 lượt bài');
  await expect(learningDebrief).toContainText('0/3');
  await expect(learningDebrief).toContainText('5');
  await expect(learningDebrief).toContainText('1');
  const debriefRows = learningDebrief.getByRole('row');
  await expect(debriefRows).toHaveCount(4);
  await expect(debriefRows.nth(1)).toContainText(/Circuit Late.*1\/6.*1.*0.*100 đ/);
  ...
  await expect(recentDebriefs).toContainText('3 học viên');
  await expect(recentDebriefs).toContainText('1 lượt chưa đạt');
  await recentDebriefs.getByRole('button').first().click();
